...
def shop_trip() -> None:
    with open('/Users/michal/Desktop/mate_academy/py-shop-trip/app/config.json', 'r') as f:
        ...
        for i in data['customers']:
            customer = Customer(i['name'], i['product_cart'], i['location'], i['money'], 
                                Car(i['car']['brand'], i['car']['fuel_consumption']))
            customer.get_money()
            list_of_shops = []
            for shop in data['shops']:
                shop_instance = Shop(shop['name'], shop['location'], shop['products'])
                list_of_shops.append(customer.trip_to_shop(shop_instance, data['FUEL_PRICE']))
            ...
            spent_money = customer.list_of_bought_stuff(the_cheapest_shop[1])
            print('')
